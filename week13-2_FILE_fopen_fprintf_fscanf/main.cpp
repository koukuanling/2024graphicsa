...
int angleID = 0;///可以視角度0、角度1、角度2、...
int oldX = 0 , oldY = 0;
#include <stdio.h>
FILE * fin = NULL;
FILE * fout = NULL;
void motion(int x, int y){ ///加入mouse motion 對應的函式
    angle[angleID] += y - oldY;///當我們的mouse motion 時,y座標的對應角度
    oldX = x;
    oldY = y;
...
void display()
{
    ...
    glEnable(GL_TEXTURE_2D);///再把貼圖功能開起來，才能dramhanda()成功

    glColor3f(1,1,1);///再把紅色的,改回白色的，手才看得清楚
